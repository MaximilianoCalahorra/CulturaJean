...
public interface ISupplyOrderRepository extends JpaRepository<SupplyOrder, Serializable>
{
	...
	@Query("SELECT so FROM SupplyOrder so INNER JOIN FETCH so.product INNER JOIN FETCH so.member INNER JOIN FETCH so.supplier "
		   + "WHERE so.supplyOrderId = (:supplyOrderId)")
	public abstract SupplyOrder findBySupplyOrderIdWithProductAndMemberAndSupplier(@Param("supplyOrderId")int supplyOrderId);
	
	//Encontramos los pedidos de aprovisionamiento de un producto determinado por su código con los objetos asociados:
	@Query("SELECT so FROM SupplyOrder so INNER JOIN FETCH so.product p INNER JOIN FETCH so.member INNER JOIN FETCH so.supplier "
		   + "WHERE p.code = (:code)")
	...
	@Query("SELECT so FROM SupplyOrder so INNER JOIN FETCH so.product INNER JOIN FETCH so.member INNER JOIN FETCH so.supplier s "
			+ "WHERE s.name = (:name)")
	...
	@Query("SELECT so FROM SupplyOrder so INNER JOIN FETCH so.product INNER JOIN FETCH so.supplier INNER JOIN FETCH so.member m "
